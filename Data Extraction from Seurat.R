...
cells <- unique(rownames(metadata))


celltype_cell <- as.data.frame(matrix(0, ncol=length(celltypes), nrow=length(cells), dimnames=list(cells, celltypes)))

for (cell in rownames(metadata)) {
  cell_type <- as.character(metadata[cell, "CellType"])
  celltype_cell[cell, cell_type] <- 1
}

HX1_meta <- subset(metadata, orig.ident == "HX1")
HX1_cell <- rownames(HX1_meta)

HX1_obs <- celltype_cell[rownames(celltype_cell) %in% HX1_cell, ]
HX2_obs <- celltype_cell[grepl("HX2", rownames(celltype_cell)), ]
HX3_obs <- celltype_cell[grepl("HX3", rownames(celltype_cell)), ]
NX1_obs <- celltype_cell[grepl("NX1", rownames(celltype_cell)), ]
NX2_obs <- celltype_cell[grepl("NX2", rownames(celltype_cell)), ]
NX3_obs <- celltype_cell[grepl("NX3", rownames(celltype_cell)), ]



write.csv(HX1_obs, "/Users/hailey/Downloads/merfish/HX1_obs.csv", row.names = TRUE)
write.csv(HX2_obs, "/Users/hailey/Downloads/merfish/HX2_obs.csv", row.names = TRUE)
write.csv(HX3_obs, "/Users/hailey/Downloads/merfish/HX3_obs.csv", row.names = TRUE)
write.csv(NX1_obs, "/Users/hailey/Downloads/merfish/NX1_obs.csv", row.names = TRUE)
write.csv(NX2_obs, "/Users/hailey/Downloads/merfish/NX2_obs.csv", row.names = TRUE)
write.csv(NX3_obs, "/Users/hailey/Downloads/merfish/NX3_obs.csv", row.names = TRUE)
